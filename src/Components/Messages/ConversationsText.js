        ...
        <div className={`p-0 h-fit ${darkMode ? 'bg-gradient-to-r from-slate-900 to-slate-700 text-white' : "bg-white"}`} ref={scrollRef}>
            <div className="">
                <div className={`${msg.to===auth.currentUser.uid?'flex':'flex flex-row-reverse'}`}>
                    <p className={`my-1 px-4 py-0.5 ${msg.to===auth.currentUser.uid?'text-left text-black bg-yellow-500 w-auto rounded-r px-2 rounded-bl-3xl':'bg-blue-400 rounded-l-lg rounded-br-3xl'}`}>{decryptedText}
